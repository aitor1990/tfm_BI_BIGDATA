...
def getFactByCountryName(factName,countryName = '',aggOperation='avg'):
    if not countryName or countryName == 'all':
        return getFactByCountries(factName,aggOperation)
    else:
        return getFactByCountry(factName,countryName,aggOperation)

def getFactByCountry(factName,countryName,aggOperation='avg'):
         query = Template(fact_by_specific_country).render(country_name = countryName,operation = aggOperation,fact = factName)
...
def getAggFactByCountry(factName,countryName,aggOperation='avg'):
    query = Template(fact_agg_by_country).render(country_name = countryName,operation = aggOperation,fact = factName)
...
def getFactByCountries(factName,aggOperation='avg'):
     query = Template(fact_by_countries).render(operation = aggOperation,fact = factName)
     results_query = drill.query(query)
     countries_name = []
     countries = []
     facts = []
